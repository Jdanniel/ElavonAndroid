...
if request.QueryString("is") <> "" then
	idSparePart = request.QueryString("is")
else
	idSparePart = 0
end if
...
if (idAR <> 0 and idTecnico <> 0 and idSparePart <> 0 and idAlmacen <> 0 and idUrgencia <> 0 and tipoServicio <> 0 and idDireccion <> 0 and fechaCompromiso <> "" and cantidad <>0) then
			...
			Do until rsValidacion.EOF
				if rsValidacion.fields("ID_STATUS_AR")<>"8" and rsValidacion.fields("ID_STATUS_AR")<>"2" then
					...
					if failed = false then
						...
						sql = sql & " @ID_TIPO_ITEM_SOLICITUD_ALMACEN=4, "
						...
						sql = sql & " @ID_SPARE_PART="&idSparePart&", "
						...
						sql = "SP_UPDATE_STATUS_AR @ID_AR = "&idAR&" ,@ID_STATUS_AR = 29 , @ID_USUARIO = "&idTecnico
						con.executeQuery(sql)
					
						response.write("<r res=""OK"" desc=""Se ha cambiado el status exitosamente!""/>")
					else
