...
sql=sql+" BD_ENVIOS.ID_TIPO_RESPONSABLE_ORIGEN, "
sql=sql+" C_TIPO_RESPONSABLE.DESC_TIPO_RESPONSABLE, "
sql=sql+" BD_ENVIOS.ID_RESPONSABLE_ORIGEN, "
...
sql=sql+" INNER JOIN C_TIPO_RESPONSABLE ON BD_ENVIOS.ID_TIPO_RESPONSABLE_ORIGEN = C_TIPO_RESPONSABLE.ID_TIPO_RESPONSABLE "
...
if rs.fields("ID_TIPO_RESPONSABLE_ORIGEN")=1 then
	sql="SELECT"
	sql=sql+" C_ALMACENES.DESC_ALMACEN "
	sql=sql+" FROM C_ALMACENES "
	sql=sql+" WHERE C_ALMACENES.ID_ALMACEN="&rs.fields("ID_RESPONSABLE_ORIGEN")
	set rs2 = con.executeQuery(sql)
	origen = rs2.fields("DESC_ALMACEN")
end if

if rs.fields("ID_TIPO_RESPONSABLE_ORIGEN")=2 then
	sql="SELECT"
	sql=sql+" C_USUARIOS.NOMBRE, "
	sql=sql+" C_USUARIOS.PATERNO, "
	sql=sql+" C_USUARIOS.MATERNO "
	sql=sql+" FROM C_USUARIOS "
	sql=sql+" WHERE C_USUARIOS.ID_USUARIO="&rs.fields("ID_RESPONSABLE_ORIGEN")
	set rs2 = con.executeQuery(sql)
		...
		if NOT isNull(rs.fields("ID_ENVIO")) then
			...
			if not rs3.eof then
				...
				do until rs3.eof
					...
					''response.write("<query>" & sql3 & "</query>")

					set rs4 = con.executeQuery(sql3)
					response.write("<query>" & rs4.EOF & "</query>")
					response.write "<p1/>"
					...
					if NOT rs4.EOF then
						...
						response.write "<p2> LLAVE = "& llave &"</p2>"
						...
						response.write "<p3> SOFTWARE = " & software &"</p3>"
