...
if request.QueryString("im") <> "" then
	idMarca = request.QueryString("im")
else
	idMarca = 0
end if

if request.QueryString("imod") <> "" then
	idModelo = request.QueryString("imod")
else
	idModelo = 0
end if
...
if (idAR <> 0 and idTecnico <> 0 and idMarca <> 0 and idModelo<>0 and idAlmacen <> 0 and idUrgencia <> 0 and tipoServicio <> 0 and idDireccion <> 0 and fechaCompromiso <> "" and cantidad <>0) then
	...
	if conFailed <> 0 then
			...
			Do until rsValidacion.EOF
				if rsValidacion.fields("ID_STATUS_AR")<>"8" and rsValidacion.fields("ID_STATUS_AR")<>"2" then
					...
					if failed = false then
						...
						sql = sql & " @ID_TIPO_ITEM_SOLICITUD_ALMACEN=3, "
						sql = sql & " @ID_SOLICITUD_ALMACEN = " & idSolicitud & " , "
						sql = sql & " @ID_CLIENTE = " & idCliente & " , "
						sql = sql & " @ID_MARCA = "&idMarca&", "
						sql = sql & " @ID_MODELO = "&idModelo&", "
						sql = sql & " @ID_INSUMO = NULL , "
						...
						sql = "SP_UPDATE_STATUS_AR @ID_AR = "&idAR&" ,@ID_STATUS_AR = 26 , @ID_USUARIO = "&idTecnico
						con.executeQuery(sql)
					
						response.write("<r res=""OK"" />")
					else
						response.write("<r res=""ERROR"" desc=""Error de Sql1: " & Err.description & """/>")
					end if
		...
		response.write("<r res=""ERROR"" desc=""Error de Sql2: " & Err.description & """/>")
	end if
else
	response.write("<r res=""ERROR"" desc=""Error de Sql3: " & Err.description & """/>")
end if
