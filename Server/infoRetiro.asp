...
if idAR <> -1 and idTecnico <> -1 then
	...
	if not rs.eof then
		...
		if not rs.eof then
			...
			sql = sql + "			 WHERE " & idAR & " = BD_RETIROS.ID_AR) = 0"
			sql = sql + "			 AND " & edit & " = 1"
			sql = sql + "			 AND ( "
			...
			''sacar los valores de las instalaciones confirmadas del negocio
			sql="SELECT"
			...
			if not rs.EOF then
				...
				response.write("<data A=""" & idNegocio & """ B=""" & idCliente & """ C=""" & isBom & """ ")
				response.write("D=""" & buttonDisplay & """ E=""" & cancelButtonDisplay & """ F=""" & edit & """ ")
				response.write("G=""" & idTipoProducto & """ />")
				
				do until rs.EOF
					response.write("<un A=""" & rs.fields("ID_UNIDAD") & """ B=""" & rs.fields("ID_MODELO") & """ ")
				...
				response.write("<r res=""OK"" val=""2"" desc=""No hay unidades en el negocio"" />")
				response.write("<data A=""" & idNegocio & """ B=""" & idCliente & """ C=""" & isBom & """ ")
				response.write("D=""" & buttonDisplay & """ E=""" & cancelButtonDisplay & """ F=""" & edit & """ ")
				response.write("G=""" & idTipoProducto & """ />")
			end if
			...
			if not rs.EOF then
				do until rs.EOF
					response.write("<ur A=""" & rs.fields("ID_UNIDAD") & """ B=""" & rs.fields("ID_MODELO") & """ ")
					response.write("C=""" & rs.fields("DESC_MODELO") & """ D=""" & rs.fields("DESC_MARCA") & """ ")
					response.write("E=""" & rs.fields("NO_SERIE") & """ F=""" & rs.fields("NO_INVENTARIO") & """ ")
					response.write("G=""" & rs.fields("NO_IMEI") & """ H=""" & rs.fields("NO_EQUIPO") & """ ")
					response.write("I=""" & rs.fields("POSICION_INVENTARIO") & """ J=""" & rs.fields("DESC_STATUS_UNIDAD") & """ ")
					response.write("K=""" & rs.fields("ID_STATUS_UNIDAD") & """ />")
